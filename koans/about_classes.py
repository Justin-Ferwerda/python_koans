...
class AboutClasses(Koan):
    ...
    def test_str_provides_a_string_version_of_the_object(self):
        ...
        self.assertEqual(fido._name, str(fido))

    def test_str_is_used_explicitly_in_string_interpolation(self):
        fido = self.Dog6("Fido")

        self.assertEqual(f"My dog is {fido.__str__()}", "My dog is " + str(fido))
    ...
    def test_all_objects_support_str_and_repr(self):
        ...
        self.assertEqual(seq.__str__(), str(seq))
        self.assertEqual(seq.__repr__(), repr(seq))
